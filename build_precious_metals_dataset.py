...
def _write_instrument(frame: pd.DataFrame, instrument: PreciousMetalInstrument) -> dict[str, object]:
    ...
    report = {
        ...
        "exact_hour_rows": int((timestamps.dt.minute.eq(0) & timestamps.dt.second.eq(0)).sum()),
        "source_policy": {
...
def build() -> dict[str, object]:
    api_key = os.environ.get("DUKASCOPY_API_KEY", "").strip()
    if not api_key:
        raise RuntimeError(
            "DUKASCOPY_API_KEY is required. Configure it as a secret; never commit it."
        )
    start, end = silver_window()
    source = DukascopyHistoricalMetalSource(api_key)
    reports = []
