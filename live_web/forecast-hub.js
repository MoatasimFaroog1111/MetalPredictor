...
function clearTiming() {
  ...
  $("prediction-interval").textContent = "غير متاح";
...
function renderEvidence(payload) {
  ...
  $("coverage").className = Number.isFinite(coverage) && coverage >= 0.9 ? "hub-good" : "hub-warn";
}
...
function renderSelection(payload) {
  ...
  $("selection-note").textContent = selection.note || "لم يجتز أي Candidate بوابة التطوير في Stage 3، لذلك بقي Random Walk هو الـBaseline البحثي المحافظ.";
}
...
function renderCollecting(payload) {
  ...
  if (payload.reason === "NO_COMPLETED_FORWARD_BAR") {
    ...
    $("state-message").textContent = "آخر Bar مكتملة لم تجتز بوابة الجودة الصارمة، لذلك لا تنشر الشاشة قيمة رقمية من هذه Bar.";
  }
}

function render(payload, horizon) {
  $("page-subtitle").textContent = `${HORIZON_LABELS[horizon]} • جميع الفترات في نفس الرابط`;
...
async function loadHorizon(horizon) {
  if (!Object.hasOwn(HORIZON_LABELS, horizon)) return;
...
$("refresh-page").addEventListener("click", () => loadHorizon(activeHorizon));
loadHorizon(activeHorizon);
