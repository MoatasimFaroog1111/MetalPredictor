...
from metal_predictor.regime_adaptive import RegimeAdaptiveConfig, RegimeAdaptiveEvaluator
from metal_predictor.walk_forward import PurgedWalkForwardSplitter, WalkForwardConfig


PROCESSED_DIR = Path("data/processed")
OUTPUT_DIR = Path("artifacts/regime_adaptive")
...
def run() -> dict[str, object]:
    ...
    baseline, specialists = model_specs()
    evaluator = RegimeAdaptiveEvaluator(
    ...
    oof.to_parquet(OUTPUT_DIR / "regime_adaptive_oof_predictions.parquet", index=False)
    oof.to_csv(OUTPUT_DIR / "regime_adaptive_oof_predictions.csv", index=False)
    (OUTPUT_DIR / "regime_adaptive_report.json").write_text(
